...
var (
	CrashAlarmInfo = AlarmLogInfo{
		EsType:       "crash-alarm",
		EsIndex:      "corerasp-crash-alarm",
		EsAliasIndex: "real-corerasp-crash-alarm",
		AlarmBuffer:  make(chan map[string]interface{}, conf.AppConfig.AlarmBufferSize),
		FileLogger:   initAlarmFileLogger("corerasp-logs/crash-alarm", "crash.log"),
	}
)
...
func parsePhpStack(alarm map[string]interface{}) (bool, map[string]interface{}) {
	...
	for idx, content := range splitCrashLogContent {
		...
		if strings.Index(content, "[0x") != -1 {
			...
			if strings.Index(content, "corerasp.so") != -1 {
				cnt += 1
				if cnt == 3 && findStack {
	...
	// 如果没有出现第三个corerasp.so, 此处需要附一个无关的值
	if alarmMessage == "" {
