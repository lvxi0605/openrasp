...
func loadIpDat() (*IpSearch, error) {
	...
	p := IpSearch{}
	//加载ip地址库信息
	data, err := ioutil.ReadFile("conf/qqzeng-ip-utf8.dat")
	if err != nil {
		log.Fatal(err)
	}
	p.data = data
...
func (p IpSearch) ParseLocaltion(localtion string) config.LocationInfo {
	var localtionInfo config.LocationInfo

	list := strings.Split(localtion, "|")
	if len(list) != 11 || list[1] == "保留" {
		localtionInfo.Continent = config.TOMLConfig.Location.Continent
		localtionInfo.Country = config.TOMLConfig.Location.Country
		localtionInfo.Province = config.TOMLConfig.Location.Province
		localtionInfo.City = config.TOMLConfig.Location.City
		localtionInfo.County = config.TOMLConfig.Location.County
		localtionInfo.ISP = config.TOMLConfig.Location.ISP
		localtionInfo.AreaCode = config.TOMLConfig.Location.AreaCode
		localtionInfo.CountryEN = config.TOMLConfig.Location.CountryEN
		localtionInfo.CountrySimple = config.TOMLConfig.Location.CountrySimple
		localtionInfo.Latitude = config.TOMLConfig.Location.Latitude
		localtionInfo.Longitude = config.TOMLConfig.Location.Longitude
		return localtionInfo
	}

	localtionInfo.Continent = list[0]
	localtionInfo.Country = list[1]
	localtionInfo.Province = list[2]
	localtionInfo.City = list[3]
	localtionInfo.County = list[4]
	localtionInfo.ISP = list[5]
	localtionInfo.AreaCode = list[6]
	localtionInfo.CountryEN = list[7]
	localtionInfo.CountrySimple = list[8]
	localtionInfo.Latitude = list[9]
	localtionInfo.Longitude = list[10]
